%Смотрим силу растяжения
close all; clear all; clc;
sigm1 = xlsread ('Vse_izmerenia.xlsx', 2, 'B:B')';
hist (sigm1, 8)
grid on
title ('Гистограмма для силы растяжения с учетом грубых ошибок')
sigm1 = sort(sigm1);
alpha = 0.11;
N = length(sigm1);
lam = linspace(sigm1(1), sigm1(N)+0.1, 5);
mu = mean(sigm1);
s = std(sigm1);
...
hist (sigm1, 8)
grid on
title ('Гистограмма для силы растяжения без учета грубых ошибок')
...
NST = length(sigm1);
t = [1.7 2.1 2.5; -1.7 -2.1 -2.5] ./ sqrt(NST);
bound = munew + snew * t
